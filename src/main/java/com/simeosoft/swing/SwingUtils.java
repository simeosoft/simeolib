 ...
 *
 * @author Simeosoft $Id: SwingUtils.java 16 2007-06-11 10:40:29Z simeo $
 */
public abstract class SwingUtils {
    ...
    public static String formatFieldToString(java.math.BigDecimal bd, int size, int dec) {
        logger.debug("(BigDecimal): entering: value: " + bd + " size: " + size + " dec: " + dec);
    ...
    public static java.sql.Date formatStringToDate(String s) {
        ...
        logger.debug("after normalize: (" + s2 + ")");
        if (s2.length() == 0) {
            return null;
        }
    ...
    public static BigDecimal formatStringToBigDecimal(String s) {
        ...
        logger.debug("formatStringToBigDecimal: trying: (" + s + ")");
     ...
     * visualizzazione errori HTML
     *
     * @param iframe internal frame di provenienza (di solito e'
     * <code>this</code>).
     * @param task task di provenienza dell'errore (di solito e'
     * <code>TASK_NAME</code>).
     * @param mess messaggio da visualizzare
    ...
    /**
     * When TAB is pressed should be possible to get the focus on the next
     * component. See com.simeosoft.form.test.JTextAreaFocusInverter for credits
     */
